...
void InitCommandLine(int argc, char** argv, po::variables_map* conf) {
  ...
  opts.add_options()
        ...
        ("use_morph_tags,M", "make MORPH tags visible to parser")
        ...
        ("morph_dim", po::value<unsigned>()->default_value(12), "MORPH dimension")
...
struct ParserBuilder {
  ...
  LookupParameters* p_m; // morphological tag embeddings
  ...
  Parameters* p_HC; // head matrix for composition function
  Parameters* p_DC; // dependency matrix for composition function

  Parameters* p_R; // relation matrix for composition function
  Parameters* p_w2l; // word to LSTM input
  Parameters* p_p2l; // POS to LSTM input

  Parameters* p_m2l; // MORPH to LSTM input
  ...
  Parameters* p_ccbias; // composition function bias for collapse nodes. --dsynt
  ...
  explicit ParserBuilder(Model* model, const unordered_map<unsigned, vector<float>>& pretrained) :
      ...
      p_HC(model->add_parameters(Dim(LSTM_INPUT_DIM, LSTM_INPUT_DIM))), //for collapsed tokens. Dsynt
      p_DC(model->add_parameters(Dim(LSTM_INPUT_DIM, LSTM_INPUT_DIM))), //for collapsed tokens. Dsynt
      ...
      p_ccbias(model->add_parameters(Dim(LSTM_INPUT_DIM, 1))), //bias for collapsed nodes.
...
//      bw_char_lstm(LAYERS, LSTM_CHAR_OUTPUT_DIM, LSTM_INPUT_DIM,  model), //Miguel
    ...
    if (USE_MORPH) {
      p_m = model->add_lookup_parameters(MORPH_SIZE, Dim(MORPH_DIM, 1));
      p_m2l = model->add_parameters(Dim(LSTM_INPUT_DIM, MORPH_DIM));
    }
...
static bool IsActionForbidden(const string& a, unsigned bsize, unsigned ssize, vector<int> stacki) {
  //seems correct for the deep-syntactic parser.
...
static map<int,int> compute_heads(unsigned sent_len, const vector<unsigned>& actions, const vector<string>& setOfActions, map<int,string>* pr = nullptr) {
  ...
  for (auto action: actions) { // loop over transitions for sentence
    ...
    if (ac =='S' && ac2=='H') {  // SHIFT
    ...
    else if (ac2=='C') { // LEFT-Collapse or RIGHT-Collapse
      ...
      heads[depi] = 999;
      rels[depi] = "collapsed";  //SAME CODE... as LA and RA with the difference that here we collapse nodes and we do not have any heads.
    }
    else {  //LEFT-ARC and RIGHT-ARC
      ...
      stacki.push_back(headi);
      heads[depi] = headi;
      rels[depi] = actionString;  //but here, we do create nodes.
...
vector<unsigned> log_prob_parser(ComputationGraph* hg,
                     ...
                     const vector<unsigned>& sentMorph,
  ...
  //cerr << endl;
    ...
    Expression HC = parameter(*hg, p_HC);
    Expression DC = parameter(*hg, p_DC);

    Expression R = parameter(*hg, p_R);
    Expression cbias = parameter(*hg, p_cbias);
    
    Expression ccbias = parameter(*hg, p_ccbias);
    ...
    for (unsigned i = 0; i < sent.size(); ++i) {
      ...
      Expression i_i;
      if (USE_MORPH && USE_POS) {
      	Expression p = lookup(*hg, p_p, sentPos[i]);
        Expression m = lookup(*hg, p_m, sentMorph[i]);
	i_i = affine_transform({ib, w2l, w, p2l, p, m2l, m});
      }else if (USE_MORPH) {
        Expression m = lookup(*hg, p_m, sentMorph[i]);
        i_i = affine_transform({ib, w2l, w, m2l, m});       
      }else if (USE_POS) {
         Expression p = lookup(*hg, p_p, sentPos[i]);
    ...
    while(stack.size() > 2 || buffer.size() > 1) {
      ...
      if (ac =='S' && ac2=='H') {  // SHIFT
      ...
      else if (ac2=='A'){ // LEFT-ARC or RIGHT-ARC
        ...
        if (headi == sent.size() - 1) rootword = intToWords.find(sent[depi])->second;
        // composed = cbias + H * head + D * dep + R * relation
        Expression composed = affine_transform({ccbias, HC, head, DC, dep}); //different matrixes.
        Expression nlcomposed = tanh(composed);
        stack_lstm.rewind_one_step();
        stack_lstm.rewind_one_step();
        stack_lstm.add_input(nlcomposed);
...
//this has to change. We need to output deep-syntactic conll with 14 columns.
...
int main(int argc, char** argv) {
  ...
  USE_MORPH = conf.count("use_morph_tags");
  ...
  //corpus.load_correct_actions_and_treebank(conf["training_data"].as<string>(),conf["training_data-dsynt"].as<string>(),conf["training_data-ssyint"].as<string>());
  /*std::cout<<corpus.nsentences<<"\n";
   std::cout<<corpus.sentences[4].size()<<"\n";
   for (int i=0;i<corpus.sentences[4].size();i++) {
        std::cout<<corpus.intToWords[corpus.sentences[4][i]]<<"\n";
   }*/
...
/*  cerr << "Number of UTF8 chars: " << corpus.maxChars << endl;
  ...
  ACTION_SIZE = corpus.nactions + 10;
  //POS_SIZE = corpus.npos + 1;
  POS_SIZE = corpus.npos + 10;
  MORPH_SIZE = corpus.nmorph + 10;
...
//  corpus.load_correct_actions_and_treebank_development_treebank_development(conf["dev_data"].as<string>());
  //corpus.load_correct_actions_and_treebank_development(conf["dev_data"].as<string>(),conf["dev_data_dsynt"].as<string>(),conf["dev_data_ssyint"].as<string>());

//        ("dev_data", po::value<string>(), "Development corpus")
//        ("dev_data_dsynt", po::value<string>(), "Development corpus deep-syntax version")
//        ("dev_data_ssynt", po::value<string>(), "Development corpus surface version")
  corpus.load_correct_actionsDev(conf["dev_data"].as<string>());
  //corpus.load_correct_actions_and_treebank_development(conf["training_data"].as<string>(),conf["training_data-dsynt"].as<string>(),conf["training_data-ssyint"].as<string>());
    ...
    while(!requested_stop) {
      ...
      for (unsigned sii = 0; sii < status_every_i_iterations; ++sii) {
	   ...
	   // std::cout<<corpus.sentences[4].size()<<"\n";
           ...
           const vector<unsigned>& dactions=corpus.correct_act_sent[order[si]];
           //std::cout<<order[si]<<"\n";
           /*for (int i=0;i<dactions.size();i++) {
		std::cout<<corpus.actions[dactions[i]]<<"\n";
           }

          for (int i=0;i<sentence.size();i++) {
                std::cout<<corpus.intToWords[sentence[i]]<<"\n";
           }*/

	   /* std::cout<<sentence.size()<<"\n";
	   std::cout<<corpus.sentences[4].size()<<"\n";
	   std::cout<<corpus.sentences[24000].size()<<"\n";  //for some reason some sentences are empty
	   std::cout<<order[si]<<"\n";*/
	   ...
	   const vector<unsigned>& sentencePos=corpus.sentencesPos[order[si]]; 
           const vector<unsigned>& sentenceMorph=corpus.sentencesMorph[order[si]];

	   const vector<unsigned>& actions=corpus.correct_act_sent[order[si]];
           ComputationGraph hg;
           parser.log_prob_parser(&hg,sentence,tsentence,sentencePos,sentenceMorph,actions,corpus.actions,corpus.intToWords,&right);
      ...
      static int logc = 0;
      ++logc;
      //if ((logc % 25 == 1) && (logc!=1)) { // report on dev set
	   ...
	   const vector<unsigned>& sentencePos=corpus.sentencesPosDev[sii]; 
           const vector<unsigned>& sentenceMorph=corpus.sentencesMorphDev[sii];
	   ...
	   if (!USE_SPELLING) {
           ...
           ComputationGraph hg;
	   vector<unsigned> pred = parser.log_prob_parser(&hg,sentence,tsentence,sentencePos,sentenceMorph, vector<unsigned>(),corpus.actions,corpus.intToWords,&right);
	   double lp = 0;
        ...
        cerr << "  **dev (iter=" << iter << " epoch=" << (tot_seen / corpus.nsentences) << ")\tllh=" << llh << " ppl: " << exp(llh / trs) << " err: " << (trs - right) / trs << " weirduas: " << (correct_heads / total_heads) << "\t[" << dev_size << " sents in " << std::chrono::duration<double, std::milli>(t_end-t_start).count() << " ms]" << endl;
  ...
  if (true) { // do test evaluation
    ...
    for (unsigned sii = 0; sii < corpus_size; ++sii) {
      ...
      if (beam_size == 1)
        pred = parser.log_prob_parser(&cg,sentence,tsentence,sentencePos,sentenceMorph,vector<unsigned>(),corpus.actions,corpus.intToWords,&right);
      llh -= lp;
      trs += actions.size();
      ...
      //here I need to call output_conll deepsyntax
      output_conll(sentence, sentencePos, sentenceUnkStr, corpus.intToWords, corpus.intToPos, hyp, rel_hyp);
      correct_heads += compute_correct(ref, hyp, sentence.size() - 1);
      total_heads += sentence.size() - 1;
    }
    auto t_end = std::chrono::high_resolution_clock::now();
    cerr << "TEST llh=" << llh << " ppl: " << exp(llh / trs) << " err: " << (trs - right) / trs << " /weirduas: " << (correct_heads / total_heads) << "\t[" << corpus_size << " sents in " << std::chrono::duration<double, std::milli>(t_end-t_start).count() << " ms]" << endl;
  }

...
public: 
   ...
   std::map<int,std::vector<unsigned>> sentencesMorph;
   ...
   std::map<int,std::vector<unsigned>> sentencesMorphDev;
   ...
   std::map<std::string, unsigned> morphToInt;
   std::map<unsigned, std::string> intToMorph;
...
inline void load_correct_actions(std::string file){
  ...
  std::vector<unsigned> current_sent_morph;
    ...
    ReplaceStringInPlace(lineS, "-LRB-", "_LRB_");
		if (lineS.empty()) {
			...
			if (!first) {
				...
				sentencesMorph[sentence] = current_sent_morph;
      }
        ...
        do {
          ...
          size_t morphIndex = word.rfind('#');
          /*if (posIndex == std::string::npos) {
            std::cerr << "cant find the dash in '" << word << "'" << std::endl;
          }*/
          /*if (morphIndex == std::string::nmorph) {
            std::cerr << "cant find the ~ in '" << word << "'" << std::endl;
          }*/

          //assert(posIndex != std::string::npos);
          //assert(morphIndex != std::string::nmorph);
 
         /* std::string morph = word.substr(morphIndex + 1);
          std::string pos = word.substr(posIndex + 1, morphIndex);
          word = word.substr(0, posIndex);*/
          std::string morph = word.substr(morphIndex + 1);
	  //std::cerr<<morph<<"\n";
          ReplaceStringInPlace(word, "#"+morph,"");
          size_t posIndex = word.rfind('-');
          if (posIndex == std::string::npos) {
            std::cerr << "cant find the dash in '" << word << "'" << std::endl;
          }
	  ...
	  std::string pos = word.substr(posIndex + 1);
          ...
          // new MORPH tag
          if (morphToInt[morph] == 0) {
            morphToInt[morph] = maxMorph;
            intToMorph[maxMorph] = morph;
            nmorph = maxMorph;
            maxMorph++;
          }
	  ...
	  current_sent_morph.push_back(morphToInt[morph]);
        } while(iss);
			}
			initial=false;
		}
...
/*	std::string oov="oov";
	...
	for (unsigned i=0;i<npos;i++){
        ...
        for (unsigned i=0;i<nmorph;i++){
                std::cerr<<i<<":"<<intToMorph[i]<<"\n";
        }
  ...
  while (getline(actionsFile, lineS)) {
    ...
    if (lineS.empty()) {
      ...
      if (!first) {
        ...
        sentencesMorphDev[sentence] = current_sent_morph;
